"""
Isomorphic Channel Sorting (ICS) — Qwen3.5-specific run script.

Qwen3.5 uses a hybrid architecture: 18/24 layers are Gated DeltaNet
(linear_attn) and 6/24 are standard GQA full attention (self_attn).
This script handles the multimodal wrapper (AutoModelForMultimodalLM),
extracts the text backbone, and runs ICS on its MLP + attention chains.
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="ICS quantize Qwen3.5 on Colab T4")
    p.add_argument("--model", default="Qwen/Qwen3.5-2B", help="HF model name or path")
    p.add_argument("--output", default="./qwen3.5-2b-ics", help="output directory")
...
def main() -> int:
    ...
    if not torch.cuda.is_available():
        print("WARNING: CUDA not available; running on CPU. This will be slow for 2B.")
        device = "cpu"
    else:
        device = "cuda"
    ...
    # Extract the text backbone — this is the Qwen3.5 language model
    # that contains all the linear chains we want to ICS-ize.
    text_model = model.language_model

    # Enable gradient checkpointing for Fisher backward pass
    if not args.no_grad_ckpt and hasattr(text_model, "gradient_checkpointing_enable"):
        text_model.gradient_checkpointing_enable()
        print("[load] gradient checkpointing ON", flush=True)

    # BitsAndBytes 4-bit requires this for backward
    if hasattr(text_model, "enable_input_require_grads"):
    ...
    # 3. Run the pipeline on the text backbone only
    t0 = time.time()
    result = quantize_model(text_model, tokenizer, config, device=device)
    ...
    for qt in result.quant.values():
        ...
        total_packed_bytes += qt.qdata.numel()
    total_dense_bytes = total_dense_params * 2
    print(f"\n=== Compression report ===")
