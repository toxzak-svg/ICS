"""Verify the dequant path: does it recover the original weight?"""
import sys, time
sys.path.insert(0, r'C:\Users\Zwmar\projects\ICS\colab_bridge')
from client import ColabClient

URL = "https://never-lecture-across-pocket.trycloudflare.com"
TOK = "KmzfnkVQWEHm1ydq-yx0J8oSeItYS5a2"
c = ColabClient(URL, TOK, timeout=300)

script = r'''
import sys, importlib
for mod_name in list(sys.modules):
    if mod_name.startswith("ics."):
        del sys.modules[mod_name]
if "ics" in sys.modules:
    del sys.modules["ics"]
sys.path.insert(0, "/content/ICS")

import torch
from ics.export import load_ics_model, dequantized_state_dict
from transformers import AutoModelForCausalLM

# Load saved
loaded = load_ics_model("/content/qwen3-0.6b-ics-gptq-int4")
sd = dequantized_state_dict(loaded)
print(f"sd has {len(sd)} layers, loaded has {len(loaded['layer_perms'])} layer_perms, {len(loaded['chain_members'])} chain_members")

# Compare to BF16 reference
ref = AutoModelForCausalLM.from_pretrained("/content/Qwen3-0.6B", dtype=torch.bfloat16, device_map="cpu")
sd_ref = ref.state_dict()
print(f"ref has {len(sd_ref)} layers")

# Compare one attention layer and one MLP layer
for name in [
    "model.layers.0.self_attn.q_proj.weight",
    "model.layers.0.self_attn.o_proj.weight",
    "model.layers.0.mlp.gate_proj.weight",
    "model.layers.0.mlp.up_proj.weight",
    "model.layers.0.mlp.down_proj.weight",
    "model.layers.5.self_attn.q_proj.weight",
    "model.layers.27.mlp.down_proj.weight",
]:
    if name not in sd:
        print(f"  {name}: MISSING from dequant")
        continue
    w_ics = sd[name].float()
    w_ref = sd_ref[name].float()
    diff = (w_ics - w_ref).abs()
    print(f"  {name}: shape={tuple(w_ics.shape)} max_diff={diff.max().item():.4f} mean_diff={diff.mean().item():.4f}  ref_std={w_ref.std().item():.4f}")

# Also check: what does the dequant look like WITHOUT the un-perm?
print()
print("--- without GPTQ + chain un-perm (just raw dequantize_blockwise) ---")
from ics.quantize import dequantize_blockwise
for name in ["model.layers.0.self_attn.q_proj.weight", "model.layers.0.mlp.gate_proj.weight"]:
    if name in loaded["layers"]:
        qt = loaded["layers"][name.replace(".weight","")]
        w_raw = dequantize_blockwise(qt)
        w_ref = sd_ref[name].float()
        diff = (w_raw - w_ref).abs()
        print(f"  {name}: raw dequant max_diff={diff.max().item():.4f} mean_diff={diff.mean().item():.4f}")
        if name.replace(".weight", "") in loaded.get("chain_members", {}).get(list(loaded["chain_members"].keys())[0], {}).get("members", []):
            print(f"    ^ member of {list(loaded['chain_members'].keys())[0]}")
'''

r = c.exec(script, timeout=300)
print(r.stdout)
if r.error:
    print("ERROR:", r.error[-2000:])
